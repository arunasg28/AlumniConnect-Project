...
const NotificationsPage = () => {
  ...
  const renderNotificationContent = (notification) => {
    switch (notification.type) {
      ...
      case "job_update":
        return notification.relatedJob ? (
          <Link
            to={`/jobs/${notification.relatedJob._id}`}
            className="flex items-center gap-2 text-blue-600 font-semibold hover:underline"
          >
            A new job has been added :{notification.relatedJob.role} at {notification.relatedJob.company}
          </Link>
        ) : (
          <span className="italic text-gray-500">[Job Unavailable]</span>
        );
      default:
        return null;
    }
